...
namespace TinySTL {
	...
	InputIterator find_if_not(InputIterator first, InputIterator last, UnaryPredicate pred) {
		...
		//********** [sort] ******************************
		//********* [Algorithm Complexity: O(NlogN)] QuickSort ****************
		namespace {
		...
		void sort(RandomIterator first, RandomIterator last, BinaryPredicate pred) {
			if (first >= last || first + 1 == last)
				return;
			if (last - first <= 20)//区间长度小于等于20的采用冒泡排序/插入排序更快
		...
		OutputIterator __copy(InputIterator first, InputIterator last, OutputIterator result, _true_type) {//内置类型利用memcpy
		...
		OutputIterator _copy(InputIterator first, InputIterator last, OutputIterator result, T*) {//指针类型
